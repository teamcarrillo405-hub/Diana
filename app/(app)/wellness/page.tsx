...
const WELLNESS_STYLES = `
  ...
  .sd-wellness-recovery { display:flex; height:max(100dvh,852px); max-height:max(100dvh,852px); container-type:inline-size; flex-direction:column; overflow:hidden; background:#0f172a; color:#fff; font-family:ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif; }
  ...
  .sd-wellness-control[data-tone="pink"] .sd-wellness-segments { gap:6px; border:0; background:transparent; padding:0; }
  .sd-wellness-control[data-tone="pink"] .sd-wellness-segments button { border:1px solid #cbd5e1; background:#fff; color:#0f172a; }
  .sd-wellness-control[data-tone="pink"] .sd-wellness-segments button[aria-pressed="true"] { border-color:#ff79da; background:#ff79da; color:#0f172a; }
  .sd-wellness-binary { grid-template-columns:repeat(2,minmax(0,1fr)); }
  ...
  .sd-wellness-quick { position:absolute; right:24px; bottom:108px; z-index:55; display:grid; width:64px; height:64px; place-items:center; border:1px solid rgb(116 192 255 / .45); border-radius:16px; background:rgb(116 192 255 / .13); color:#74c0ff; box-shadow:0 10px 30px rgb(116 192 255 / .18); }
  ...
  .sd-wellness-submit { width:100%; min-height:51px; border:1px solid rgb(116 192 255 / .45); border-radius:12px; background:rgb(116 192 255 / .13); color:#74c0ff; font-size:11px; font-style:italic; font-weight:950; letter-spacing:.14em; text-transform:uppercase; box-shadow:0 0 20px rgb(116 192 255 / .18); }
  ...
  .sd-wellness-movement-panel { margin-top:24px; border:1px solid #cbd5e1; border-radius:16px; background:#f4efe6; padding:17px; color:#0f172a; box-shadow:0 10px 24px rgb(2 6 23 / .14); }
  .sd-wellness-movement-form { display:grid; gap:12px; }
  .sd-wellness-movement-form h2,.sd-wellness-movement-form h3 { margin:0; color:#0f172a; font-size:15px; font-style:italic; font-weight:950; text-transform:uppercase; }
  .sd-wellness-movement-types,.sd-wellness-felt { display:flex; flex-wrap:wrap; gap:7px; }
  .sd-wellness-movement-types button,.sd-wellness-felt button { min-height:32px; border:1px solid #cbd5e1; border-radius:8px; background:#f8fafc; padding:6px 10px; color:#0f172a; font-size:9px; font-weight:850; text-transform:uppercase; }
  .sd-wellness-movement-types button[aria-pressed="true"] { border-color:#0ea5e9; background:#e0f2fe; color:#0369a1; }
  .sd-wellness-felt button[aria-pressed="true"] { border-color:#0ea5e9; background:#e0f2fe; color:#0369a1; }
  .sd-wellness-minutes { display:grid; grid-template-columns:auto minmax(0,1fr) auto; align-items:center; gap:10px; color:#475569; font-size:9px; font-weight:850; letter-spacing:.08em; text-transform:uppercase; }
  .sd-wellness-minutes input { width:100%; accent-color:#0ea5e9; }
  .sd-wellness-minutes output { min-width:34px; color:#0369a1; font-size:12px; font-style:italic; font-weight:950; text-align:right; }
  .sd-wellness-movement-submit { min-height:43px; border:1px solid #0ea5e9; border-radius:10px; background:#e0f2fe; color:#0369a1; font-size:10px; font-style:italic; font-weight:950; letter-spacing:.1em; text-transform:uppercase; }
  .sd-wellness-movement-submit:disabled { background:#f1f5f9; color:#64748b; }
  ...
  .diana-app .sd-wellness-control[data-tone="pink"] .sd-wellness-segments button { border:1px solid #cbd5e1; background:#fff; color:#0f172a; }
  .diana-app .sd-wellness-control[data-tone="pink"] .sd-wellness-segments button[aria-pressed="true"] { border-color:#ff79da; background:#ff79da; color:#0f172a; }
  .diana-app .sd-wellness-quick { display:grid; width:64px; min-height:64px; border:1px solid rgb(116 192 255 / .45); border-radius:16px; background:rgb(116 192 255 / .13); padding:0; color:#74c0ff; box-shadow:0 10px 30px rgb(116 192 255 / .18); }
  .diana-app .sd-wellness-submit { display:flex; width:100%; min-height:51px; border:1px solid rgb(116 192 255 / .45); border-radius:12px; background:rgb(116 192 255 / .13); padding:0 16px; color:#74c0ff; box-shadow:0 0 20px rgb(116 192 255 / .18); }
  ...
  .diana-app .sd-wellness-movement-types button,.diana-app .sd-wellness-felt button { display:inline-flex; width:auto; min-height:32px; border:1px solid #cbd5e1; border-radius:8px; background:#f8fafc; padding:6px 10px; color:#0f172a; box-shadow:none; }
  .diana-app .sd-wellness-movement-types button[aria-pressed="true"],.diana-app .sd-wellness-felt button[aria-pressed="true"] { border-color:#0ea5e9; background:#e0f2fe; color:#0369a1; }
  .diana-app .sd-wellness-movement-submit { display:flex; width:100%; min-height:43px; border:1px solid #0ea5e9; border-radius:10px; background:#e0f2fe; padding:0 12px; color:#0369a1; box-shadow:none; }
  .diana-app .sd-wellness-movement-submit:disabled { width:auto; min-height:34px; border-color:#cbd5e1; background:#fff; padding:0 11px; color:#64748b; font-size:8px; letter-spacing:.07em; }
  @container (min-width:768px) {
    .sd-wellness-header,.sd-wellness-scroll,.sd-wellness-footer { padding-left:clamp(32px,6vw,96px); padding-right:clamp(32px,6vw,96px); }
    .sd-wellness-header-row,.sd-wellness-header h1,.sd-wellness-header p { width:min(100%,760px); margin-left:auto; margin-right:auto; }
    .sd-wellness-scroll > section,.sd-wellness-scroll > .sd-wellness-status,.sd-wellness-footer .sd-wellness-submit { width:min(100%,760px); margin-left:auto; margin-right:auto; }
  }
  @media (min-width:1100px) {
    .diana-app .sd-source-viewport.sd-wellness-recovery { width:100%; height:auto; max-width:none; max-height:none; min-height:100dvh; overflow:visible; }
    .diana-app .sd-wellness-recovery .sd-student-desktop-nav { position:relative; }
    .diana-app .sd-wellness-header { padding-top:2.5rem; background:transparent; backdrop-filter:none; }
    .diana-app .sd-wellness-header-row { display:none; }
    .diana-app .sd-wellness-header h1 { font-size:3.25rem; }
    .diana-app .sd-wellness-scroll { flex:none; overflow:visible; padding-bottom:24px; }
    .diana-app .sd-wellness-footer { padding-top:0; background:transparent; backdrop-filter:none; }
  }
...
export default async function WellnessPage() {
  ...
  const [{ data: profile }, { data: sleepLogs }] =
    await Promise.all([
      supabase
        .from("profiles")
        .select("session_mood, display_name, photo_url, photo_offset_x, photo_offset_y")
        .eq("user_id", user.id)
        .maybeSingle(),
      supabase
        .from("sleep_logs")
