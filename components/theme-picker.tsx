...
export function ThemePicker() {
  ...
  return (
    <div
      role="group"
      aria-label="Theme"
      className="diana-control-group inline-flex items-center gap-1 rounded-2xl border border-border bg-surface-soft p-1"
    >
...
export function ThemeQuickToggle({ className = '' }: { className?: string }) {
  ...
  return (
    <button
      ...
      title={label}
      className={`diana-rail-link touch-target inline-flex flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2 text-[11px] font-semibold text-muted transition hover:bg-surface-soft hover:text-fg ${className}`}
    >
      <Icon size={18} />
      <span>Theme</span>
    </button>
